...
export default function HistoryRescuePage() {
  ...
  useEffect(() => {
    ...
    const fetchCases = async () => {
      try {
        const terminalStates = ['resolved', 'completed', 'cancelled'];
        ...
        (snapshot || []).forEach(data => {
          fetched.push({
            ...
            status: data.status,
            timeCompleted: data.resolved_at ? new Date(data.resolved_at).toLocaleString('th-TH') : 
                           (data.updated_at ? new Date(data.updated_at).toLocaleString('th-TH') : '-'),
            timestamp: data.resolved_at ? new Date(data.resolved_at).getTime() : 
        ...
        console.error('Error fetching rescue history:', err);
        setLoading(false);
      }
    };
    ...
    const channel = supabase
      .channel('custom-history-rescue')
      .on(
        'postgres_changes',
        { event: '*', schema: 'public', table: 'cases', filter: `assigned_volunteer_id=eq.${uid}` },
        () => {
          fetchCases();
        }
      )
