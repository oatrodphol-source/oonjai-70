...
const getTriageColor = (severity: number | string) => {
...
// Alias for the cluster solid color
const getSeveritySolidColor = getTriageColor;
...
export default function MapView({ cases: propCases }: { cases?: CasePoint[] } = {}) {
  const position: [number, number] = [13.7563, 100.5018]; // BKK Default
  const [fetchedCases, setFetchedCases] = useState<CasePoint[]>([]);
  const [viewMode, setViewMode] = useState<'markers' | 'heatmap'>('markers');

  const activeCases = propCases && propCases.length > 0 ? propCases : fetchedCases;

  useEffect(() => {
    if (propCases && propCases.length > 0) return;
    ...
    const intervalId = setInterval(fetchCasesData, 5000);
    return () => clearInterval(intervalId);
  }, [propCases]);

  const validCases = Array.isArray(activeCases) ? activeCases : [];
        ...
        {(() => {
          ...
          validCases.forEach(c => {
            ...
            validCases.forEach(other => {
              const isCompleted = ['resolved', 'completed', 'cancelled'].includes((other as any).status);
          ...
          return clusters.map((cluster, idx) => {
            ...
            if (cluster.group.length > 1) {
              const maxSeverity = Math.max(...cluster.group.map(c => {
                const match = String(c.severity || (c as any).level || 1).match(/\d+/);
