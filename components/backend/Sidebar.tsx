...
export const Sidebar = ({ role = 'admin', userName = 'สมหมาย ใจดี' }: { role?: string, userName?: string }) => {
  ...
  const menuItems = [
    ...
    { href: '/news', icon: Newspaper, label: 'ข่าวสาร', roles: ['admin'] },
    { href: '/report-dashboard', icon: FileText, label: 'รายงาน', roles: ['admin', 'rescue', 'volunteer'] },
    { href: '/admin/history', icon: History, label: 'ประวัติช่วยเหลือ', roles: ['rescue', 'volunteer'] },
    { href: '/users', icon: Users, label: 'จัดการผู้ใช้', roles: ['admin'] },
