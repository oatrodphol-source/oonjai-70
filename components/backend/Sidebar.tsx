...
export const Sidebar = ({ role = 'volunteer', userName = 'กำลังตรวจสอบ...' }: { role?: string, userName?: string }) => {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel('cases-insert-channel')
      .on(
        'postgres_changes',
        { event: 'INSERT', schema: 'public', table: 'cases' },
        (payload) => {
          // Use Web Audio API for a reliable beep without needing an MP3 file
          try {
            // Optional Toast or Alert
            console.log("🚨 SOS เข้าใหม่!", payload.new);
          } catch (e) {
  ...
  const menuItems = [
    ...
    { href: '/water-data', icon: CloudRain, label: 'ข้อมูลภัยพิบัติ', roles: ['admin', 'rescue', 'volunteer'] },
    ...
    { href: '/safe-reports', icon: UserCheck, label: 'จัดการผู้ปลอดภัย', roles: ['admin', 'rescue'] },
    ...
    <>
      ...
      <div className="flex-1 overflow-y-auto py-6 px-4 flex flex-col gap-2">
        {filteredMenu.map((item) => {
          ...
          if (currentRole === 'admin' && item.href === '/profile') return null;
      ...
      <div className="mt-auto border-t border-slate-700/50 p-4">
        {(currentRole === 'rescue' || currentRole === 'volunteer') && (
          <div className="mb-4 w-full flex justify-center">
            <VolunteerStatusToggle />
          </div>
        )}
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col truncate">
            ...
            <span className="text-xs text-slate-400 flex items-center gap-1">
              ...
              {currentUser?.role === 'admin' ? 'แอดมิน' : currentUser?.role === 'volunteer' ? 'อาสาสมัคร' : (currentUser?.role || 'Online')}
            </span>
          </div>
          <button onClick={handleLogout} title="ออกจากระบบ" className="ml-auto p-2 text-slate-400 hover:text-white hover:bg-slate-800 rounded-lg transition-colors flex-shrink-0 cursor-pointer">
